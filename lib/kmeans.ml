...
let has_converged old_clusters new_clusters distance_fn threshold =
  List.for_all2
    (fun old_c new_c -> distance_fn old_c new_c < threshold)
...
let run_kmeans k points distance_fn =
  if points = [] then invalid_arg "Points list cannot be empty."
    ...
    let rec run old_clusters new_clusters i =
      ...
      if has_converged old_clusters centroid_updated distance_fn 0.0001 then
        centroid_updated
